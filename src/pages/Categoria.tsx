              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {hasActiveFilters && (
                  <button
                    ...
                    className="text-sm text-[#4513b8] hover:text-[#360fa0]"
                  >
                    Limpiar
                  </button>
                )}
                ...
                <Sheet open={showFilters} onOpenChange={setShowFilters}>
                  <SheetTrigger asChild>
                    <Button variant="outline" className="lg:hidden border-gray-300 text-sm">
                      ...
                      Filtros
                      {hasActiveFilters && (
                        <Badge className="ml-2 bg-[#4513b8]">!</Badge>
                      )}
                    </Button>
                  </SheetTrigger>
                  <SheetContent side="left" className="w-80 bg-white border-gray-200">
                    ...
                    <div className="mt-6 space-y-6">
                      ...
                      <Button onClick={() => setShowFilters(false)} className="w-full bg-[#4513b8] hover:bg-[#360fa0]">
                        Aplicar filtros
                      </Button>
                    </div>
                  </SheetContent>
...
function ProductoCategoriaCard({ producto, viewMode = 'grid' }: ProductoCategoriaCardProps) {
  ...
  if (viewMode === 'list') {
    return (
      <Link to={`/producto/${producto.id}`}>
        <div className="flex gap-3 md:gap-4 bg-white border border-gray-200 rounded-xl overflow-hidden group hover:border-[#4513b8] hover:shadow-lg transition-all p-3 md:p-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="font-semibold text-gray-900 text-sm md:text-base mb-1 md:mb-2 group-hover:text-[#4513b8] transition-colors">
              {producto.nombre}
            </h3>
            <p className="text-gray-500 text-xs md:text-sm line-clamp-2 mb-2 md:mb-3">{producto.descripcion}</p>
            <div className="flex items-end gap-2">
              <span className="text-lg md:text-xl font-bold text-[#4513b8]">
  ...
  return (
    <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
      <Link to={`/producto/${producto.id}`}>
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden group hover:border-[#4513b8] hover:shadow-lg transition-all h-full">
          <div className="relative">
            ...
            {producto.etiquetas.includes('Nuevo') && (
              <Badge className="absolute top-2 right-2 bg-[#4513b8] text-xs">Nuevo</Badge>
            )}
          </div>
          <div className="p-2 md:p-4">
            ...
            <div className="flex items-end gap-1 md:gap-2">
              <span className="text-base md:text-lg font-bold text-[#4513b8]">
                {formatPrice(producto.precio)}
              </span>
              {producto.precioAnterior && (
