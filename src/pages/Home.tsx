...
import { ArrowRight, Zap, TrendingUp, Star, ChevronLeft, ChevronRight, Headphones, Watch, Cable, Mouse } from 'lucide-react';
...
const heroSlides = [
  {
    id: 1,
    titulo: 'Audífonos con Cancelación de Ruido',
    subtitulo: 'Sonido de nivel profesional',
    descripcion: 'Cancelación activa de ruido, 30 horas de batería. La experiencia auditiva definitiva.',
    imagen: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=1200',
    cta: 'Ver ahora',
    link: '/producto/au-001',
    color: 'from-violet-600 to-blue-600',
  },
  {
    id: 2,
    titulo: 'Smartwatch Deportivo con GPS',
    subtitulo: 'Tu salud, siempre contigo',
    descripcion: 'GPS integrado, 20 modos deportivos y 7 días de batería. Para cada aventura.',
    imagen: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=1200',
    cta: 'Comprar',
    link: '/producto/sw-001',
    color: 'from-cyan-600 to-blue-600',
  },
  {
    id: 3,
    titulo: 'Cargadores GaN 65W',
    subtitulo: 'Carga rápida, donde vayas',
    descripcion: 'Tecnología GaN ultracompacta. Carga tu laptop, tablet y celular al mismo tiempo.',
    imagen: 'https://images.unsplash.com/photo-1585338107529-13afc5f02586?w=1200',
    cta: 'Ver oferta',
    link: '/producto/cg-002',
      ...
      <section className="py-8 md:py-16">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between mb-6 md:mb-8">
            ...
            <Link to="/categorias">
              <Button variant="ghost" className="text-[#4513b8] hover:text-[#360fa0] text-sm md:text-base">
                Ver todas
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 gap-3 md:gap-4">
            {categorias.map((categoria, index) => {
              const iconMap: Record<string, React.ReactNode> = {
                'Headphones': <Headphones className="w-7 h-7 md:w-8 md:h-8" />,
                'Watch': <Watch className="w-7 h-7 md:w-8 md:h-8" />,
                'Zap': <Zap className="w-7 h-7 md:w-8 md:h-8" />,
                'Mouse': <Mouse className="w-7 h-7 md:w-8 md:h-8" />,
                'Cable': <Cable className="w-7 h-7 md:w-8 md:h-8" />,
              };
              return (
                <motion.div
                  key={categoria.id}
                ...
                >
                  <Link to={`/categoria/${categoria.id}`}>
                    <Card className="bg-white border-gray-200 hover:border-[#4513b8] hover:shadow-lg transition-all group overflow-hidden">
                      <CardContent className="p-3 md:p-5 text-center">
                        <div className="w-14 h-14 md:w-16 md:h-16 mx-auto mb-2 md:mb-3 rounded-2xl bg-[#4513b8]/10 flex items-center justify-center text-[#4513b8] group-hover:bg-[#4513b8] group-hover:text-white transition-all">
                          {iconMap[categoria.icono] ?? <Zap className="w-7 h-7" />}
                        </div>
                        <h3 className="font-medium text-gray-900 group-hover:text-[#4513b8] transition-colors text-xs md:text-sm">
                          {categoria.nombre}
                        </h3>
                      </CardContent>
                    </Card>
      ...
      <section className="py-8 md:py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-2 md:gap-3 mb-6 md:mb-8">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-[#4513b8]/10 rounded-xl flex items-center justify-center">
              <Zap className="w-4 h-4 md:w-5 md:h-5 text-[#4513b8]" />
            </div>
            <div>
